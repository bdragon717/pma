...
import { ContactItemData, TelNoViewProps } from '@/src/utils/types';
...
const TelNoView = (props: TelNoViewProps) => {
  ...
  const onSubmit = async (formData: InputFormValues) => {
    ...
    const filteredContacts: ContactItemData[] = Object.values(
          data.filter(contact => contact.name && contact.name.toLowerCase().includes(searchName))
              .reduce((acc: any, contact) => { // 배열의 각 요소에 대해 주어진 리듀서(reducer) 함수를 실행하고, 하나의 결과값(누적값)을 반환
    ...
    <View style={styles.itemsd}>
      ...
      {item.phoneNumber.map((number, index) => (
        <Pressable key={index}
                   ...
                   style={{ alignSelf: 'flex-start' }} // 텍스트 길이만큼만 영역 잡기
        >
  ...
  // TelNo Modal Open
  const handleTelNoIsOpen = () => {
    props.setTelNoIsOpen(false);
  };
  ...
  return (
    <View style={styles.containersn}>
      ...
      <View style={styles.headerst}>
        <Text style={styles.headerText}>전화번호 찾기</Text>
      </View>
      <View style={styles.serchft}>
        <Controller
            control={control}
            name="inputText"
            render={({ field: { onChange, onBlur, value } }) => (
              <TextInput
                style={[styles.input, {width: width*0.45, height: height*0.05} ]}           
                value={value}
